...
anno <- read.table("data/SCANB/1_clinical/raw/Project51_ERp_AB_transformed_SPSSselection.txt",sep="\t",fill=TRUE, header = TRUE)
str(anno) #ET #Chemotherapy #AI #Tamoxifen #HER2 #ER_SPSS #Molecular_subtype, #Relapse #Days_until_relapse #distant_metastasis
View(anno)

# SCANB NPJ release
df <- as.data.frame(read_excel("data/SCANB/1_clinical/raw/NPJ_release.xlsx"))
View(df)
str(df) #ER #HER2 #TretGroup #DRFi_days #DRFi_event #NCN.PAM50 

# 2.2 extract relevant variables
survdata <- anno %>% dplyr::select(rba_rel4, fuV8,
